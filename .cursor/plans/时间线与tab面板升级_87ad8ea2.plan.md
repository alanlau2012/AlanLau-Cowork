...
function saveCollapsedState() {
  ...
  const collapsed = Array.from(document.querySelectorAll('.inline-tool-call:not(.expanded)')).map(
...
function restoreCollapsedState() {
  ...
  const collapsed = JSON.parse(localStorage.getItem(`collapsed_${currentChatId}`) || '[]');
